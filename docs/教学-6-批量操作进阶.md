...
func (db *DB) BatchCompleteTodosPartialContext(ctx context.Context, ids []int) (*BatchResult, error) {
	...
	for _, id := range ids {
		...
		if rowErr != nil {
			...
			result.Errors = append(result.Errors, BatchError{
				ID:    id,
				Error: fmt.Sprintf("获取受影响行数失败: %v", rowErr),
			})
			continue
		}
		if rowsAffected == 0 {
...
```go
func (db *DB) BatchCompleteTodosPartialContext(ctx context.Context, ids []int) (*BatchResult, error) {
    // ... 与 BatchDeleteTodosPartialContext 类似 ...
...
**核心要点**：
1. Context 支持：`BeginTx`、`ExecContext`、循环中检查 `ctx.Done()`
2. 部分失败：`continue` 跳过失败，记录 `BatchError`
3. 事务提交：即使有失败，成功的也要 `Commit`
