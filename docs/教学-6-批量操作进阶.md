# 教学 Part 6: 批量操作进阶（部分成功策略）

> **📌 学习阶段**: 中级进阶 / Go 数据库编程
> **前置要求**: 已完成教学-5（事务基础 + 全有或全无策略）
> **学习目标**: 实现部分成功策略的批量操作，理解两种策略的权衡
> **时间投入**: 2-3 小时
...
> **注意**：本节是教学-5（全有或全无策略）的进阶版本，实现**部分成功策略**。
> 函数命名带 `Context` 后缀，与项目现有代码风格保持一致。
...
func (db *DB) BatchCompleteTodosPartialContext(ctx context.Context, ids []int) (result *BatchResult, err error) {
	...
	// 使用 BeginTx 支持 Context（与教学-5一致）
	tx, err := db.conn.BeginTx(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to begin transaction: %w", err)
	}

	// 使用 defer 确保事务被处理（与教学-5一致）
	defer func() {
		if err != nil {
...
```
- 使用 `continue` 跳过失败的 ID（教学-5 使用 `return`）
- 记录每个失败的原因
...
> **注意**：Handler 代码与教学-5保持一致的 Context 超时控制模式。
> 使用项目已有的 `BatchTimeout` 常量（在教学-5中添加）。

**第 1 步**：确保 `handler/handler.go` 已有 `BatchTimeout` 常量（教学-5已添加）：
```go
const (
...
**第 2 步**：添加批量操作 handlers（使用部分成功策略）：

```go
// BatchRequest 批量操作请求（复用教学-5的结构体）
// 如果教学-5未添加，需要在此添加
...
> **注意**：教学-5已添加基础批量操作路由（全有或全无策略）。
> 本节添加部分成功策略的路由（使用不同端点区分）。

**选项 A**：替换原有端点（如果只需要部分成功策略）
...
- 参考当前项目的 `api/routes.go:58-68` 的注册顺序
...
**简述**：在循环中使用 `:=` 声明 `err` 会创建新变量，遮蔽外层的 `err`，导致 defer 中的回滚逻辑永远不会执行。

**错误代码**：
```go
for _, id := range ids {
    result, err := tx.ExecContext(ctx, ...)  // ❌ := 创建新变量
    rows, err := result.RowsAffected()       // ❌ 同样遮蔽
}
```

**正确做法**：
1. 使用命名返回值 `(result *BatchResult, err error)`
2. 预先声明变量 `var res sql.Result` 和 `var rowsAffected int64`
3. 循环内使用 `=` 赋值

**本节示例的做法**：

本节的 `BatchCompleteTodosPartialContext` 与教学-5 保持一致，使用命名返回值：
```go
func (db *DB) BatchCompleteTodosPartialContext(ctx context.Context, ids []int) (result *BatchResult, err error) {
    // ...
    var res sql.Result
    ...
    for _, id := range ids {
        res, err = tx.ExecContext(ctx, ...)  // ✅ 用 = 赋值
        if err != nil {
            // 记录错误...
            err = nil  // ✅ 重置 err，部分成功策略不回滚
            continue
        }
        rowsAffected, err = res.RowsAffected()  // ✅ 用 = 赋值
        // ...
    }
}
```

**部分成功策略的特殊之处**：记录错误后需要 `err = nil` 重置，避免触发 defer 回滚。
...
> **前置条件**：已完成教学-5（全有或全无策略的批量操作）。
> 本节在其基础上扩展为部分成功策略。

**第 1 步**：回顾教学-5的实现（全有或全无）
...
func (db *DB) BatchDeleteTodosPartialContext(ctx context.Context, ids []int) (result *BatchResult, err error) {
    ...
    for _, id := range ids {
        ...
        if err != nil {
            result.FailedCount++
            result.Errors = append(result.Errors, BatchError{ID: id, Error: err.Error()})
            err = nil  // 重置 err，部分成功策略
            continue   // 关键区别：继续处理下一个
        }
...
// 注意：使用命名返回值
func (db *DB) BatchCompleteTodosPartialContext(ctx context.Context, ids []int) (result *BatchResult, err error) {
    // ... 与 BatchDeleteTodosPartialContext 类似的初始化 ...
...
**Context 支持**（与教学-5一致）：
- [ ] 使用 `BeginTx(ctx, nil)` 而不是 `Begin()`
- [ ] 使用 `ExecContext(ctx, ...)` 而不是 `Exec(...)`
- [ ] 在循环中检查 `ctx.Done()`（允许中断）
- [ ] Handler 层使用 `context.WithTimeout` 创建超时
- [ ] 区分 `context.DeadlineExceeded` 和 `context.Canceled`
...
**安全性**：
- [ ] 批量大小限制（最多 100 个，Handler 和 DB 双重校验）
- [ ] 使用参数化查询（无 SQL 注入风险）
- [ ] IDs 为空时返回 400 错误
...
**代码质量**：
- [ ] 使用命名返回值 `(result *BatchResult, err error)`
- [ ] 预先声明循环变量 `var res sql.Result` 和 `var rowsAffected int64`
- [ ] 循环内使用 `=` 赋值，不使用 `:=`
- [ ] 记录错误后 `err = nil` 重置（部分成功策略特有）
- [ ] 函数命名带 `Context` 后缀（与项目风格一致）
- [ ] 错误处理完善
- [ ] 有清晰的注释
- [ ] 代码格式符合 gofmt
...
> **注意**：以下脚本假设你选择了**选项 A**（替换原有端点）。
> 如果选择**选项 B**（保留两种策略），请将端点改为 `/batch/complete-partial` 和 `/batch/delete-partial`。
...
保存为 `scripts/test-batch-partial.sh`，然后执行：
```bash
chmod +x scripts/test-batch-partial.sh
./scripts/test-batch-partial.sh
```

> **提示**：如果教学-5的测试脚本是 `scripts/test-batch.sh`，
> 建议将本节的脚本命名为 `scripts/test-batch-partial.sh` 以区分。
...
现在请你按照以上步骤（假设教学-5已完成）：

1. **修改 `database/db.go`**：
   - 添加 `BatchError` 和 `BatchResult` 结构体
   - 实现 `BatchCompleteTodosPartialContext()` 函数（部分成功策略）
   - 实现 `BatchDeleteTodosPartialContext()` 函数（部分成功策略）

2. **修改 `handler/handler.go`**：
   - 复用教学-5的 `BatchRequest` 结构体
   - 实现 `BatchCompleteTodosPartial` handler（带 Context 超时）
   - 实现 `BatchDeleteTodosPartial` handler（带 Context 超时）

3. **修改 `api/routes.go`**：
   - 选择替换原有端点或添加新端点（见选项 A/B）
   - 注意路由顺序（在 `/{id}` 之前）

4. **测试**：
   - 使用上面的测试脚本验证功能
   - 确保部分失败、空列表、超限、超时等边界情况都正确处理

**记住**（与教学-5的区别）：
- **相同点**：都使用命名返回值 `(err error)` 让 defer 能访问错误
- 使用 `continue` 跳过失败项（而非 `return` 回滚）
- 记录错误后 `err = nil` 重置，避免触发 defer 回滚
- 使用 `RowsAffected()` 区分"执行失败"和"ID 不存在"
- 即使有失败，也要 `Commit` 成功的操作
- 返回 `BatchResult` 而非 `error`

**与教学-5的对比**：
| 特性 | 教学-5（全有或全无） | 教学-6（部分成功） |
|------|---------------------|-------------------|
| 函数签名 | `(err error)` | `(result *BatchResult, err error)` |
| 失败处理 | 任何失败都回滚 | 记录失败，`err = nil` 重置，继续处理 |
| 返回值 | `error` | `*BatchResult, error` |
| 用户体验 | "1个失败，全部取消" | "19个成功，1个失败" |
| 复杂度 | 简单 | 中等 |
...
**教学-5 vs 教学-6**：
- 教学-5：全有或全无策略（简单，适合强一致性场景）
- 教学-6：部分成功策略（复杂，适合用户友好场景）

**何时使用哪种策略？**
- **全有或全无**：转账、订单创建等需要强一致性的场景
- **部分成功**：批量删除、批量标记等用户操作，允许部分失败

**核心要点**：
1. **命名返回值**：使用 `(result *BatchResult, err error)`，让 defer 能访问到错误
2. **Context 支持**：`BeginTx`、`ExecContext`、循环中检查 `ctx.Done()`
3. **部分失败**：`continue` 跳过失败，记录 `BatchError`，然后 `err = nil` 重置
4. **事务提交**：即使有部分失败，成功的也要 `Commit`
