...
> **📌 学习阶段**: 中级 / Go I/O 编程
> **前置要求**: 已完成教学-5/6（事务、Context、批量操作）
...
> **注意**：函数命名带 `Context` 后缀，与项目现有代码风格保持一致（参考教学-5/6）。
...
func (db *DB) ImportTodosContext(ctx context.Context, todos []model.Todo) (imported int, err error) {
    ...
    // 使用 BeginTx 支持 Context（与教学-5/6一致）
    ...
    // 预先声明 stmt，避免使用 := 带来的潜在混淆（与教学-5/6风格一致）
    var stmt *sql.Stmt
...
> **注意**：Handler 层添加 Context 超时控制，与项目现有 handlers 保持一致。

**第 1 步**：确保超时常量已添加（如果教学-5/6未添加）：
```go
const (
    // ... 其他超时常量 ...
...
**第 2 步**：在 `handler/handler.go` 中添加导出功能:
...
在 `api/routes.go` 的 `registerTodoRoutes` 闭包中添加（参考当前项目结构 `api/routes.go:58-68`）:
...
**路由顺序说明**：
- `/export` 和 `/import` 必须在 `/{id}` 之前注册
- 与 `/stats`、`/batch/*` 路由放在同一区域
...
> **关键点**：详见教学-5"常见陷阱"第6条（变量遮蔽导致 defer 失效）。
...
**代码质量**（与教学-5/6一致）:
- [ ] 使用命名返回值 `(imported int, err error)`
- [ ] 预先声明变量 `var stmt *sql.Stmt`
- [ ] 循环内使用 `=` 赋值，不使用 `:=`
- [ ] `ctx.Done()` 分支正确赋值 `err = ctx.Err()` 后再 return

**Context 支持**（与教学-5/6一致）:
- [ ] 使用 `BeginTx(ctx, nil)` 而不是 `Begin()`
- [ ] 使用 `QueryContext`、`ExecContext`、`PrepareContext`
- [ ] 在循环中检查 `ctx.Done()` 并正确赋值给命名返回值
- [ ] Handler 层使用 `context.WithTimeout`
...
**第 1 步**: 添加超时常量（如果教学-5/6未添加）
- 在 `handler/handler.go` 中添加 `ExportTimeout` 和 `ImportTimeout`

**第 2 步**: 实现 `database/db.go` 中的 `ImportTodosContext` 和 `ExportTodosContext`
- 使用命名返回值 `(imported int, err error)`
- 预先声明变量 `var stmt *sql.Stmt`
- 使用 `BeginTx(ctx, nil)` 开启事务
- 使用 `PrepareContext`、`ExecContext`、`QueryContext`
- 在循环中检查 `ctx.Done()` 并正确赋值 `err = ctx.Err()`

**第 3 步**: 实现 `handler/handler.go` 中的导出功能
- 添加 Context 超时控制
- 区分超时/取消/其他错误

**第 4 步**: 实现导入功能(先 JSON，后 CSV)
- 同样添加 Context 超时控制

**第 5 步**: 注册路由
- 在 `/{id}` 之前注册 `/export` 和 `/import`
...
**与教学-5/6的一致性**：
- **命名返回值**：使用 `(imported int, err error)` 让 defer 能访问错误
- **预先声明变量**：`var stmt *sql.Stmt`，循环内用 `=` 赋值
- **ctx.Done() 处理**：正确赋值 `err = ctx.Err()` 后再 return
- **函数命名**：`xxxContext` 后缀
- **事务处理**：`BeginTx` + `defer Rollback`
- **超时控制**：`context.WithTimeout` + 错误区分

**现在，开始实现导入导出功能吧！记住：命名返回值、流式处理、事务保证、Context 超时、安全验证。** 🚀
