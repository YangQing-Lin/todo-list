...
func (db *DB) BatchCompleteTodos(ids []int) (*BatchResult, error) {
	...
	for _, id := range ids {
		// 更新状态为 completed，同时设置 completed_at
		res, execErr := tx.Exec(`
			UPDATE todos
...
**5. 批量完成的特殊逻辑**（第 48-54 行）
```go
UPDATE todos
SET status = 'completed',
    completed_at = datetime('now'),  // 设置完成时间
    updated_at = datetime('now')
WHERE id = ? AND status = 'pending'  // 只更新 pending 状态的
```
...
**路由注册顺序注意**：
- `POST /api/v1/todos/batch/complete` 必须在 `PUT /api/v1/todos/{id}` 之前
- 否则 `batch/complete` 会被当作 `{id}` 参数（值为 "batch/complete"）
